...
//	Hide();
 ...
 //	settings.antialiasingLevel = 8;
	...
	t.loadFromFile(DIRECTION + "ForGameRats\\Rats.jpg");
	Texture Backgroundfontexture;
	Backgroundfontexture.loadFromFile(DIRECTION + "ForGameRats\\Фон.png");
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			else if ((event.type == Event::MouseButtonPressed) && (GameOver) && (event.key.code == Mouse::Right))
			{
				using namespace std;
				std::ofstream file;
				file.open("ResultGameRAt.txt");
				if (file.is_open())
				{
		...
		if (GameOver)
		{
			...
			Font font;//шрифт 
			font.loadFromFile(DIRECTION + "ForGameRats\\Шрифты\\CyrilicOld.ttf");//передаем нашему шрифту файл шрифта
			...
			Texture GOver;
			GOver.loadFromFile(DIRECTION + "ForGameRats\\GameOver.png");
			Sprite GOverSprite;
